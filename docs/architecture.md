...
|---|---|---|---|
| MCP | `watch-skill serve` | stdio (default) or streamable HTTP (`--http`, port 8747, endpoint `/mcp`) | 28 tools ([reference](tools/README.md)); responses are text + real image blocks capped at `response_frame_cap`. Structured errors serialize as `{error, message, fix, details}`. |
