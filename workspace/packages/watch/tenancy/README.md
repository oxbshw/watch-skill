...
```sh
npm install @deepwatch/dsh-tenancy
```
...
Classify an action for the audit ledger, without writing a secret into it:

```ts
import { credentialUse, looksLikeSecret } from '@deepwatch/dsh-tenancy'

if (looksLikeSecret(value)) value = '[redacted]'
const entry = credentialUse({ actor, provider })
```
