...
Agree a protocol version before either side sends anything:

```ts
import {
  negotiateProtocol, WATCH_PROTOCOL_VERSION, isSuccessVerdict,
} from '@deepwatch/dsh-contracts'

const agreed = negotiateProtocol(WATCH_PROTOCOL_VERSION)
if (isSuccessVerdict(reply.verdict)) { /* the check passed */ }
```
