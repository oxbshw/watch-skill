 ...
 * is not hypothetical, and the version it happened against is part of the
 * point: Watch Skill 1.2.0 -- `latest` on PyPI when this was written -- ships
 * `serve`, `api` and a CLI, and no `bridge` subcommand at all. (1.4.x does
 * ship one; that is exactly why the engine on a machine cannot be assumed from
 * the version this repository builds.) Pinning `stdio` against it turned an honest
...
export function writeCoreBinOverride(profileDir: string, coreBin: string): string {
  ...
  writeFileSync(path, [
    ...
    '#',
    '# The timeouts are deliberately absent. This block used to restate them,',
    '# which made it a second copy of a boundary the bundle patch already',
    '# declares — and the copy that wins, because it is written last. Raising',
    '# the startup budget to 45s in the bundle therefore changed nothing for',
    '# any profile built by `deepwatch setup`: the clean room kept the ten',
    '# seconds that reported a healthy first start as a dead engine, and the',
    '# fix looked applied while the composed profile disagreed.',
    '#',
    ...
    '#',
    '# `cwd` stays empty, and that is now a statement rather than a shrug. The',
    '# launcher starts the Host in the canonical workspace and exports',
    '# `DEEPWATCH_WORKSPACE` beside it, so an engine inheriting the Host cwd',
    '# inherits the workspace a person chose. Core does not depend on that: it',
    '# reads the variable, and fails closed with a named fix when nobody',
    '# established one. Before either existed, this empty value is how the',
    '# engine, the agent tools and the verifier each ended up in a different',
    '# directory, and a file written correctly could not be verified.',
