...
Compare two runs of one contract and ask whether their verdicts differ:

```ts
import {
  compareProjections, hasVerdictDivergence, comparisonDigest,
} from '@deepwatch/dsh-trajectory'

const comparison = compareProjections(left, right)
if (hasVerdictDivergence(comparison)) {
  console.log('they disagree', comparisonDigest(comparison))
}
```
