...
export function notes(inventory, manifest) {
  ...
  if (bundle !== undefined) {
    say(
      ...
      '',
      '> **Installing from the attached files takes fourteen of them, not one.**',
      `> \`${BUNDLE}\` names thirteen siblings as ordinary registry dependencies,`,
      '> so pointing `dsh plugin add` at the bundle tarball on its own sends pnpm',
      '> to the registry for the rest — which is not what an offline install',
      '> wants. Pass the bundle **and** the thirteen it depends on:',
      '>',
      '> ```bash',
      `> dsh plugin --profile web add ./${bundle.file} \\`,
      '>   ./deepwatch-dsh-client-brand-*.tgz ./deepwatch-dsh-client-evidence-*.tgz \\',
      '>   ./deepwatch-dsh-client-memory-*.tgz ./deepwatch-dsh-client-remotes-*.tgz \\',
      '>   ./deepwatch-dsh-client-settings-*.tgz ./deepwatch-dsh-core-bridge-*.tgz \\',
      '>   ./deepwatch-dsh-library-*.tgz ./deepwatch-dsh-live-*.tgz \\',
      '>   ./deepwatch-dsh-memory-*.tgz ./deepwatch-dsh-technology-*.tgz \\',
      '>   ./deepwatch-dsh-tools-*.tgz ./deepwatch-dsh-trajectory-*.tgz \\',
      '>   ./deepwatch-dsh-workspace-*.tgz',
      '> ```',
      '>',
      '> `deepwatch setup --artifacts <dir>` does the equivalent for the whole',
      '> product, and is the easier route when you want all of it.',
      '')
  }
