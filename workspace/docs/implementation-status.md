...
Baseline: DeepSeek Harness `0.1.1-rc.2 @ b150a551b8d465e31e418e1b2eaf5e79bbb7d28e`, Watch Core `watch-skill 1.4.0rc1, Bridge protocol 1`.
...
- Typert generates the Node host and remote client faces; Pydantic schema digests still provide the cross-language drift gate.
...
- The browser gate covers the composed slot occupancy and first-run identity.
...
- The provider form remains DSH-owned; DeepWatch contributes the tested unified Role Bindings editor.
- Credential references point at DSH; no Watch credential store exists, by design.

**`technology.center`**

- GPU-specific engine quality remains external hardware validation; no unmeasured quality number is rendered.
...
- Integrity digests are over first-party package source. They identify a build; they are not a signature, and signing is PRODUCTION SIGNING NOT PROVEN.
- SPDX licences are declared, never concluded: nobody has audited the files to conclude anything stronger.
