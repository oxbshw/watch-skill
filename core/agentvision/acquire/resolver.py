...
from agentvision.acquire import cache, cobalt, direct, ytdlp
...
def _resolve_local(source: str) -> AcquireResult:
    ...
    if path.suffix.lower() not in VIDEO_EXTS:
        print(
            f"[agentvision] warning: {path.suffix} is not a known video extension — proceeding",
            file=sys.stderr,
        )
...
def _from_cache(source: str, kind: SourceKind) -> AcquireResult | None:
    ...
    if entry is None or entry.video_path is None:
        return None
    print(f"[agentvision] cache hit: {entry.dir}", file=sys.stderr)
    return AcquireResult(
        source=source,
        kind=kind,
...
def _try_chain(
    ...
    if kind == SourceKind.PAGE_URL and cobalt.is_configured():
        try:
            ...
            print(f"[agentvision] cobalt failed ({exc.code}) — trying direct ffmpeg…", file=sys.stderr)
    ...
    raise AcquisitionError(
        ...
        fix="check the URL; if it needs login or is region-locked, AgentVision "
...
def acquire(
    ...
    if kind in (SourceKind.SCREEN, SourceKind.WINDOW):
        raise AcquisitionError(
            ...
            fix="use `agentvision capture` / the capture() API (Milestone 3)",
