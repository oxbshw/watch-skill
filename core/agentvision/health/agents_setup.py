"""Detect installed AI agents and write AgentVision's MCP config into them.

The killer onboarding move: ``agentvision setup`` finds every supported agent
...
class AgentTarget:
    ...
    configured: bool = False  # already has an agentvision entry
...
def server_command(project_dir: Path | None = None) -> tuple[str, list[str]]:
    ...
    `agentvision` that resolves inside a virtualenv is invisible to other
    apps, so a source checkout always gets the `uv --directory` form."""
    if project_dir is not None:
        return "uv", ["--directory", str(project_dir), "run", "agentvision", "serve"]
    exe = shutil.which("agentvision")
    if exe and ".venv" not in Path(exe).parts and "venv" not in Path(exe).parts:
        return "agentvision", ["serve"]
    root = Path(__file__).resolve().parents[3]
    return "uv", ["--directory", str(root), "run", "agentvision", "serve"]
...
def _write_json_mcpservers(path: Path, command: str, args: list[str]) -> None:
    ...
    servers["agentvision"] = {"command": command, "args": args}
...
def _write_toml_codex(path: Path, command: str, args: list[str]) -> None:
    ...
    if "[mcp_servers.agentvision]" in text:
        return
...
def configure_agent(
    ...
    """Write the agentvision MCP entry into one agent's config.
