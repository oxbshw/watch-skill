...
def dense_resample(video: dict, hits: list[Hit]) -> tuple[int, int]:
    ...
    settings = get_settings()
    try:
        from agentvision.acquire import acquire  # noqa: PLC0415
        from agentvision.perceive import perceive  # noqa: PLC0415
    ...
    for center in centers:
        ...
        try:
            ...
            print(f"[agentvision] escalation resample failed ({exc.code})", file=sys.stderr)
            continue
...
def zoom_crops_reocr(video: dict, hits: list[Hit]) -> tuple[int, int]:
    ...
    from agentvision.answer import crops  # noqa: PLC0415
    ...
    for timestamp, frame_path in frames:
        try:
            ...
            print(f"[agentvision] zoom crops skipped ({exc.code})", file=sys.stderr)
            return new_items, 0
        if blocks:
...
def _insert_ocr_blocks(video_id: str, timestamp: float, frame_path: str, blocks: list) -> int:
    from agentvision.index.db import connect  # noqa: PLC0415
    from agentvision.index.store import _index_texts  # noqa: PLC0415
...
def estimate_verify_cost(n_frames: int, prompt_text: str, width: int = 512) -> int:
    ...
    from agentvision.answer.types import est_text_tokens  # noqa: PLC0415
