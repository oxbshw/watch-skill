...
Optional and flag-gated (``AGENTVISION_DIARIZATION_ENABLED`` or
``--diarize``). The heavy backend (pyannote.audio, torch) is an optional
extra — ``pip install agentvision[diarize]`` — and needs a Hugging Face token
...
def diarize_audio(audio_path: Path, hf_token: str | None = None) -> list[SpeakerTurn]:
    ...
    try:
        ...
        raise TranscriptionError(
            ...
            fix="pip install 'agentvision[diarize]' (needs torch; ~2 GB)",
        ) from exc
    if not token:
        raise TranscriptionError(
...
def diarize_transcript(
    ...
    if not transcript:
        return transcript
    try:
        from agentvision.transcribe.audio import extract_audio  # noqa: PLC0415
