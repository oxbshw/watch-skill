...
def transcribe_local(
    ...
    try:
        ...
        raise TranscriptionError(
            ...
            fix='install the whisper extra: `uv sync --extra whisper` or `pip install "agentvision[whisper]"`',
        ) from exc
    ...
    print(
        f"[agentvision] local whisper: model={size} device={device} ({compute})…",
        file=sys.stderr,
    )
    try:
        ...
        raise TranscriptionError(
            ...
            fix="try a smaller model (AGENTVISION_WHISPER_MODEL=tiny) or enable cloud STT",
