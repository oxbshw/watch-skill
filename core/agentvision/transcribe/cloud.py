...
def _require_cloud_enabled() -> None:
    if not get_settings().cloud_stt_enabled:
        raise TranscriptionError(
            ...
            fix="set AGENTVISION_CLOUD_STT_ENABLED=1 to opt in — only extracted "
...
def pick_backend(preferred: str | None = None) -> tuple[str, str]:
    ...
    raise TranscriptionError(
        ...
        fix="set AGENTVISION_GROQ_API_KEY (preferred) or AGENTVISION_OPENAI_API_KEY",
    )
    ...
    with audio_path.open("rb") as fh:
        response = httpx.post(
            ...
            headers={
                ...
                "User-Agent": "agentvision/0.1 (+https://github.com/agentvision)",
            },
...
def _post_with_retry(endpoint: str, api_key: str, model: str, audio_path: Path) -> dict:
    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        if attempt < MAX_ATTEMPTS - 1:
            print(f"[agentvision] cloud STT retry in {delay:.1f}s…", file=sys.stderr)
            time.sleep(delay)
    raise TranscriptionError(
...
def transcribe_cloud(
    ...
    if size <= audio_mod.MAX_UPLOAD_BYTES:
        ...
        print(
            f"[agentvision] audio {size / 1024**2:.0f} MB — {len(plan)} overlapping chunks…",
            file=sys.stderr,
        )
        ...
        for i, (chunk_path, offset) in enumerate(
            ...
            try:
                ...
                failures += 1
                print(f"[agentvision] chunk {i + 1}/{len(plan)} failed: {exc.code}", file=sys.stderr)
